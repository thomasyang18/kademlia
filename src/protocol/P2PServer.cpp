...
void P2PServer::handleMessage(const Message& msg) {
    switch (msg.type) {
        ...
        case MessageType::FIND_NODE:
            udpNode_.sendMessage({MessageType::FIND_NODE_REPLY, udpNode_.getMyNode(), msg.target, udpNode_.getRoutingTable().findClosestNodes(msg.target, Config::().kBucketSize)}, msg.sender.ip, msg.sender.port);
            break;
